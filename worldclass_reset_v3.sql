...
-- =========================================
-- V3 EXTENSIONS · CONTROL CENTER / ACTOR TRACKING / DYNAMIC SETUP
-- =========================================

alter table public.app_settings
  add column if not exists labels jsonb not null default '{"vehicle": "Auto", "driver": "Pilota", "event": "Evento", "turn": "Turno"}'::jsonb;

create table if not exists public.team_setup_fields (
  id uuid primary key default gen_random_uuid(),
  team_id uuid not null references public.teams(id) on delete cascade,
  field_key text not null,
  label text not null,
  group_name text not null default 'Generale',
...
create table if not exists public.team_dashboard_widgets (
  ...
  created_at timestamptz not null default now(),
  unique(team_id, role_scope, widget_code)
);

create table if not exists public.event_car_drivers (
  id uuid primary key default gen_random_uuid(),
  team_id uuid not null references public.teams(id) on delete cascade,
  event_car_id uuid not null references public.event_cars(id) on delete cascade,
  driver_id uuid not null references public.drivers(id) on delete cascade,
  role text not null default 'primary',
  created_at timestamptz not null default now(),
  unique(event_car_id, driver_id)
);

alter table public.event_car_turns
  add column if not exists driver_id uuid references public.drivers(id) on delete set null,
  add column if not exists fuel_start_liters numeric not null default 0,
  add column if not exists fuel_end_liters numeric not null default 0,
  add column if not exists created_by_team_user_id uuid references public.team_users(id) on delete set null;

alter table public.car_components
  add column if not exists mounted_by_team_user_id uuid references public.team_users(id) on delete set null,
  add column if not exists removed_by_team_user_id uuid references public.team_users(id) on delete set null,
  add column if not exists reason text;

alter table public.component_revisions
  add column if not exists created_by_team_user_id uuid references public.team_users(id) on delete set null;

alter table public.maintenances
  add column if not exists created_by_team_user_id uuid references public.team_users(id) on delete set null,
  add column if not exists assigned_to_team_user_id uuid references public.team_users(id) on delete set null;
...
alter table public.documents
  ...
  add column if not exists uploaded_by_team_user_id uuid references public.team_users(id) on delete set null;
...
alter table public.telemetry_files
  add column if not exists storage_path text,
...
create index if not exists idx_team_setup_fields_team_id on public.team_setup_fields(team_id);
create index if not exists idx_team_dashboard_widgets_team_id on public.team_dashboard_widgets(team_id);
create index if not exists idx_event_car_drivers_event_car_id on public.event_car_drivers(event_car_id);
create index if not exists idx_event_car_turns_driver_id on public.event_car_turns(driver_id);
...
alter table public.team_setup_fields enable row level security;
alter table public.team_dashboard_widgets enable row level security;
alter table public.event_car_drivers enable row level security;

create policy team_setup_fields_select_team on public.team_setup_fields for select using (public.is_team_member(team_id));
create policy team_setup_fields_insert_team on public.team_setup_fields for insert with check (public.is_team_member(team_id));
create policy team_setup_fields_update_team on public.team_setup_fields for update using (public.is_team_member(team_id)) with check (public.is_team_member(team_id));
create policy team_setup_fields_delete_team on public.team_setup_fields for delete using (public.is_team_member(team_id));

create policy team_dashboard_widgets_select_team on public.team_dashboard_widgets for select using (public.is_team_member(team_id));
create policy team_dashboard_widgets_insert_team on public.team_dashboard_widgets for insert with check (public.is_team_member(team_id));
create policy team_dashboard_widgets_update_team on public.team_dashboard_widgets for update using (public.is_team_member(team_id)) with check (public.is_team_member(team_id));
create policy team_dashboard_widgets_delete_team on public.team_dashboard_widgets for delete using (public.is_team_member(team_id));

create policy event_car_drivers_select_team on public.event_car_drivers for select using (public.is_team_member(team_id));
create policy event_car_drivers_insert_team on public.event_car_drivers for insert with check (public.is_team_member(team_id));
create policy event_car_drivers_update_team on public.event_car_drivers for update using (public.is_team_member(team_id)) with check (public.is_team_member(team_id));
create policy event_car_drivers_delete_team on public.event_car_drivers for delete using (public.is_team_member(team_id));
...
begin
  v_auth_user := auth.uid();
  if v_auth_user is null then raise exception 'Utente non autenticato'; end if;
  if exists (select 1 from public.team_users where auth_user_id = v_auth_user and is_active = true) then
    raise exception 'Utente già associato a un team attivo';
  end if;

  v_vehicle_type := coalesce(nullif(p_vehicle_type,''), 'auto');

  insert into public.teams (name, slug, vehicle_type)
  values (p_team_name, regexp_replace(lower(p_team_name), '[^a-z0-9]+', '-', 'g'), v_vehicle_type)
  returning id into v_team_id;
  ...
  insert into public.app_settings (team_id, team_name, team_subtitle, vehicle_type)
  values (v_team_id, p_team_name, 'Gestione motorsport', v_vehicle_type);

  delete from public.team_component_definitions where team_id = v_team_id;
  if v_vehicle_type = 'moto' then
    insert into public.team_component_definitions (team_id, code, label, category, is_required, tracks_hours, has_expiry, default_expiry_years, order_index) values
    (v_team_id, 'motore', 'Motore', 'base', true, true, false, null, 1),
    (v_team_id, 'forcella', 'Forcella', 'base', true, true, false, null, 2),
    (v_team_id, 'mono', 'Mono ammortizzatore', 'base', true, true, false, null, 3),
    (v_team_id, 'freni', 'Freni', 'base', true, true, false, null, 4),
    (v_team_id, 'passaporto', 'Passaporto tecnico', 'expiry', true, false, true, 10, 5);
  elseif v_vehicle_type = 'kart' then
    insert into public.team_component_definitions (team_id, code, label, category, is_required, tracks_hours, has_expiry, default_expiry_years, order_index) values
    (v_team_id, 'motore', 'Motore', 'base', true, true, false, null, 1),
    (v_team_id, 'freni', 'Freni', 'base', true, true, false, null, 2),
    (v_team_id, 'sterzo', 'Sterzo', 'base', true, true, false, null, 3),
  ...
  insert into public.team_setup_fields (team_id, field_key, label, group_name, field_type, unit, position, order_index) values
  (v_team_id, 'pressione_ant_sx', 'Pressione ant. sx', 'Pneumatici', 'number', 'bar', 'left', 1),
  (v_team_id, 'pressione_ant_dx', 'Pressione ant. dx', 'Pneumatici', 'number', 'bar', 'left', 2),
  (v_team_id, 'pressione_post_sx', 'Pressione post. sx', 'Pneumatici', 'number', 'bar', 'right', 3),
  (v_team_id, 'pressione_post_dx', 'Pressione post. dx', 'Pneumatici', 'number', 'bar', 'right', 4),
  (v_team_id, 'note_setup', 'Note setup', 'Generale', 'textarea', null, 'center', 5);

  insert into public.team_dashboard_widgets (team_id, role_scope, widget_code, label, size, order_index) values
  (v_team_id, 'all', 'cars_ready', 'Mezzi pronti', 'md', 1),
  (v_team_id, 'all', 'components_alerts', 'Componenti critici', 'md', 2),
  (v_team_id, 'all', 'upcoming_events', 'Prossimi eventi', 'md', 3),
  (v_team_id, 'all', 'maintenances_open', 'Manutenzioni aperte', 'md', 4)
  on conflict (team_id, role_scope, widget_code) do nothing;
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'team_files_read') then
    create policy team_files_read on storage.objects for select to authenticated using (bucket_id = 'team-files');
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'team_files_insert') then
    create policy team_files_insert on storage.objects for insert to authenticated with check (bucket_id = 'team-files');
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'team_files_update') then
    create policy team_files_update on storage.objects for update to authenticated using (bucket_id = 'team-files') with check (bucket_id = 'team-files');
  end if;
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'team_files_delete') then
    create policy team_files_delete on storage.objects for delete to authenticated using (bucket_id = 'team-files');
  end if;
end $$;

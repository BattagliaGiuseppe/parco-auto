create or replace function public.set_connected_circuit_geofence(
  p_team_id uuid,
  p_circuit_id uuid,
  p_latitude numeric,
  p_longitude numeric,
  p_detection_radius_m integer
)
returns jsonb
language plpgsql
security definer
...
begin
  if not public.has_team_permission(p_team_id,'events.edit') then
    raise exception 'Permesso events.edit richiesto';
  end if;
  select * into c from public.circuits where id=p_circuit_id and team_id=p_team_id;
  if not found then raise exception 'Circuito non valido per il team'; end if;

  if p_latitude is null and p_longitude is null and p_detection_radius_m is null then
    update public.circuits set latitude=null,longitude=null,detection_radius_m=null where id=c.id;
    return jsonb_build_object('circuit_id',c.id,'configured',false);
  end if;

  if p_latitude is null or p_longitude is null then raise exception 'Latitudine e longitudine sono entrambe obbligatorie'; end if;
  if p_latitude < -90 or p_latitude > 90 then raise exception 'Latitudine non valida'; end if;
  if p_longitude < -180 or p_longitude > 180 then raise exception 'Longitudine non valida'; end if;
  if coalesce(p_detection_radius_m,0) < 50 or p_detection_radius_m > 10000 then
    raise exception 'Raggio geofence non valido (50-10000 m)';
  end if;

  update public.circuits
  set latitude=p_latitude,longitude=p_longitude,detection_radius_m=p_detection_radius_m
  where id=c.id;

  return jsonb_build_object('circuit_id',c.id,'configured',true,'latitude',p_latitude,'longitude',p_longitude,'detection_radius_m',p_detection_radius_m);
end;
$function$;

revoke all on function public.set_connected_circuit_geofence(uuid,uuid,numeric,numeric,integer) from public, anon;
grant execute on function public.set_connected_circuit_geofence(uuid,uuid,numeric,numeric,integer) to authenticated, service_role;

    ...
    <div className={`grid gap-3 ${columnsClassName}`}>
      {items.map((item) => (
        <div
          ...
          className={`rounded-2xl border border-neutral-200 bg-neutral-50 p-4 ${item.cardClassName || ""}`}
        >
          ...
          <div className={`mt-2 text-xl font-bold ${item.valueClassName || "text-neutral-900"}`}>
            {item.value}
          </div>

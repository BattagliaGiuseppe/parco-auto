...
import RoutingAdminPanel from "@/components/connected/RoutingAdminPanel";
...
type DeviceRow = {
  ...
  serial_number?: string | null; external_device_id?: string | null; source_type: string; acquisition_mode: AcquisitionMode; session_authority?: "smartphone" | "external_logger"; status: string;
...
type StreamWindowRow = { id: string; external_window_id: string; started_at: string; ended_at: string; samples_count: number; segments_count: number; status: string; error_message?: string | null; device_name: string; car_name: string; track_segments: number; engine_only_segments: number };
type StreamStatus = { stats: { windows_30d: number; segments_30d: number }; recent_windows: StreamWindowRow[] };
...
export default function ConnectedDevicesPage() {
  ...
  const [streamStatus, setStreamStatus] = useState<StreamStatus>({ stats: { windows_30d: 0, segments_30d: 0 }, recent_windows: [] });
  ...
  const [form, setForm] = useState({ carId: "", name: "", provider: "generic", model: "", serial: "", externalId: "", sourceType: "logger", acquisitionMode: "external_logger" as AcquisitionMode, defaultDriverId: "" });
  ...
  const load = useCallback(async () => {
    ...
    const [pageResult, streamResult] = await Promise.all([
      supabase.rpc("connected_devices_page", { p_team_id: access.ctx.teamId }),
      supabase.rpc("connected_stream_status", { p_team_id: access.ctx.teamId }),
    ]);
    if (pageResult.error) setError(pageResult.error.message); else setBundle((pageResult.data || {}) as PageBundle);
    if (!streamResult.error) setStreamStatus((streamResult.data || { stats: { windows_30d: 0, segments_30d: 0 }, recent_windows: [] }) as StreamStatus);
    setLoading(false);
  ...
  async function createDevice() {
    ...
    if (result.device?.id) {
      const { error: modeError } = await supabase.rpc("set_connected_device_acquisition_mode", { p_team_id: access.ctx.teamId, p_device_id: result.device.id, p_mode: form.acquisitionMode });
      if (modeError) setError(modeError.message);
    }
  ...
  async function setDeviceAcquisitionMode(device: DeviceRow, mode: AcquisitionMode) {
    if (!access.ctx?.teamId) return;
    setError(null);
    const { error } = await supabase.rpc("set_connected_device_acquisition_mode", {
      p_team_id: access.ctx.teamId, p_device_id: device.id, p_mode: mode,
    });
    if (error) setError(error.message); else await load();
  }
  ...
  return <div className="space-y-6 p-4 text-neutral-900 dark:text-neutral-100 md:p-6">
    ...
    <section className="rounded-2xl border border-neutral-200 bg-white text-neutral-900 shadow-sm dark:border-neutral-800 dark:bg-neutral-950 dark:text-neutral-100">
      <div className="border-b border-neutral-200 px-4 py-3 dark:border-neutral-800"><div className="font-black">Finestre stream automatiche</div><div className="mt-1 text-xs text-neutral-500">P2.4 segmenta un flusso continuo in Solo motore / Pista senza salvare i campioni GPS uno per uno.</div></div>
      <div className="grid gap-3 p-4 sm:grid-cols-2"><div className="rounded-xl border border-neutral-200 p-3 dark:border-neutral-800"><div className="text-xs uppercase text-neutral-500">Finestre 30 gg</div><div className="mt-1 text-2xl font-black">{streamStatus.stats.windows_30d}</div></div><div className="rounded-xl border border-neutral-200 p-3 dark:border-neutral-800"><div className="text-xs uppercase text-neutral-500">Segmenti generati 30 gg</div><div className="mt-1 text-2xl font-black">{streamStatus.stats.segments_30d}</div></div></div>
      {streamStatus.recent_windows.length>0&&<div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-neutral-50 text-left text-xs uppercase text-neutral-500 dark:bg-neutral-900"><tr><th className="px-4 py-3">Data</th><th className="px-4 py-3">Mezzo</th><th className="px-4 py-3">Campioni</th><th className="px-4 py-3">Segmenti</th><th className="px-4 py-3">Pista</th><th className="px-4 py-3">Solo motore</th><th className="px-4 py-3">Stato</th></tr></thead><tbody>{streamStatus.recent_windows.map((w)=><tr key={w.id} className="border-t border-neutral-200 dark:border-neutral-800"><td className="px-4 py-3">{fmtDate(w.started_at)}</td><td className="px-4 py-3"><div className="font-semibold">{w.car_name}</div><div className="text-xs text-neutral-500">{w.device_name}</div></td><td className="px-4 py-3">{w.samples_count}</td><td className="px-4 py-3">{w.segments_count}</td><td className="px-4 py-3">{w.track_segments}</td><td className="px-4 py-3">{w.engine_only_segments}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-bold ${w.status==='processed'?'bg-emerald-100 text-emerald-700':w.status==='failed'?'bg-red-100 text-red-700':'bg-amber-100 text-amber-800'}`}>{w.status==='processed'?'Elaborata':w.status==='failed'?'Errore':'In elaborazione'}</span>{w.error_message&&<div className="mt-1 text-xs text-red-600">{w.error_message}</div>}</td></tr>)}</tbody></table></div>}
    </section>

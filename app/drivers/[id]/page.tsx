...
import { ArrowLeft, UserRound, ShieldCheck, FileText, PlusCircle } from "lucide-react";
...
export default function DriverDetailPage() {
  ...
  if (access.loading) {
    return <PagePermissionState title="Scheda pilota" subtitle="Documenti, licenze e storico tecnico" icon={<UserRound size={22} />} state="loading" />;
  }
  if (access.error) {
    return <PagePermissionState title="Scheda pilota" subtitle="Documenti, licenze e storico tecnico" icon={<UserRound size={22} />} state="error" message={access.error} />;
  }
  if (!canViewDrivers) {
    return <PagePermissionState title="Scheda pilota" subtitle="Documenti, licenze e storico tecnico" icon={<UserRound size={22} />} state="denied" message="Il tuo ruolo non può aprire la scheda pilota." />;
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      {!canEditDrivers ? <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">Licenze e documenti sono in sola lettura per il tuo ruolo.</div> : null}
